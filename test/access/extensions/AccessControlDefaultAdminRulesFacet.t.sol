...
contract AccessControlDefaultAdminRulesFacetTest is DiamondBase, AccessControlFacetTest {
    AccessControlDefaultAdminRulesMock accessControlDAR;
    uint48 constant DELAY = 10 hours;

    function setUp() public virtual override(AccessControlFacetTest, DiamondBase) {
        DiamondBase.setUp();

        accessControlDAR = new AccessControlDefaultAdminRulesMock();
        ...
        accessControlDAR = AccessControlDefaultAdminRulesMock(diamond);
    }

    function _mock() internal view virtual override returns (address) {
        return address(accessControlDAR);
    }
    ...
    function test_defaultAdmin() public view {
        assertEq(AccessControlDefaultAdminRulesMock(_mock()).defaultAdmin(), owner);
        assertEq(AccessControlDefaultAdminRulesMock(_mock()).owner(), owner);
        assertEq(
            AccessControlDefaultAdminRulesMock(_mock()).defaultAdmin(),
            AccessControlDefaultAdminRulesMock(_mock()).owner()
        );
        assertTrue(AccessControlDefaultAdminRulesMock(_mock()).hasRole(DEFAULT_ADMIN_ROLE, owner));
    }

    function test_defaultAdminTransfer() public {
        // Start an admin transfer
        AccessControlDefaultAdminRulesMock(_mock()).beginDefaultAdminTransfer(alice);

        // Wait for the transfer to be accepted
        vm.warp(DELAY + 2);
        vm.prank(alice);
        AccessControlDefaultAdminRulesMock(_mock()).acceptDefaultAdminTransfer();

        assertEq(AccessControlDefaultAdminRulesMock(_mock()).defaultAdmin(), alice);
        assertEq(AccessControlDefaultAdminRulesMock(_mock()).owner(), alice);
        assertTrue(AccessControlDefaultAdminRulesMock(_mock()).hasRole(DEFAULT_ADMIN_ROLE, alice));
    }

    function test_pendingDefaultAdminReturnsZeroIfNoPendingDefaultAdminTransfer() public view {
        (address newAdmin, uint48 schedule) = AccessControlDefaultAdminRulesMock(_mock()).pendingDefaultAdmin();
    ...
    function test_pendingDefaultAdminReturnsPendingAdminAndScheduleWhenTransferScheduled() public {
        ...
        for (uint8 i; i < 3; ++i) {
            ...
            (, uint48 firstSchedule) = AccessControlDefaultAdminRulesMock(_mock()).pendingDefaultAdmin();
    ...
    function test_defaultAdminDelayIsCorrect() public view {
        assertEq(AccessControlDefaultAdminRulesMock(_mock()).defaultAdminDelay(), DELAY);
    }
    ...
    function test_defaultAdminDelayReturnsAppropriateValue() public {
        ...
        for (uint8 i; i < 3; ++i) {
            AccessControlDefaultAdminRulesMock(_mock()).changeDefaultAdminDelay(newDelay);

            // Wait until schedule + fromSchedule
            (, uint48 schedule) = AccessControlDefaultAdminRulesMock(_mock()).pendingDefaultAdminDelay();
            vm.warp(uint256(int256(int48(schedule)) + fromSchedule[i]));

            uint48 currentDelay = AccessControlDefaultAdminRulesMock(_mock()).defaultAdminDelay();
            assertEq(currentDelay, expectNew[i] ? newDelay : DELAY);
        }
    }

    function test_pendingDefaultAdminDelayReturnsZeroIfNotSet() public view {
        (uint48 delay, uint48 schedule) = AccessControlDefaultAdminRulesMock(_mock()).pendingDefaultAdminDelay();
        assertEq(delay, 0);
        assertEq(schedule, 0);
    }
    ...
    function test_pendingDefaultAdminDelayReturnsAppropriateValue() public {
        ...
        for (uint8 i; i < 3; ++i) {
            ...
            // Wait until schedule + fromSchedule
            (, uint48 firstSchedule) = AccessControlDefaultAdminRulesMock(_mock()).pendingDefaultAdminDelay();
            vm.warp(uint256(int256(int48(firstSchedule) + fromSchedule[i])));
    ...
    function test_defaultAdminDelayIncreaseWaitReturns5Days() public view {
        assertEq(AccessControlDefaultAdminRulesMock(_mock()).defaultAdminDelayIncreaseWait(), 5 days);
    }
    ...
    function testRevert_grantDefaultAdminRole() public {
        ...
        AccessControlDefaultAdminRulesMock(_mock()).grantRole(DEFAULT_ADMIN_ROLE, alice);
    }

    function testRevert_revokeDefaultAdminRole() public {
        vm.expectRevert(IAccessControlDefaultAdminRules.AccessControlEnforcedDefaultAdminRules.selector);
        AccessControlDefaultAdminRulesMock(_mock()).revokeRole(DEFAULT_ADMIN_ROLE, alice);
    }

    function testRevert_cannotChangeDefaultAdminRoleAdmin() public {
        vm.expectRevert(IAccessControlDefaultAdminRules.AccessControlEnforcedDefaultAdminRules.selector);
        AccessControlDefaultAdminRulesMock(_mock())._internal_setRoleAdmin(DEFAULT_ADMIN_ROLE, ROLE);
    }

    function testRevert_cannotGrantDefaultAdminRoleTwice() public {
        vm.expectRevert(IAccessControlDefaultAdminRules.AccessControlEnforcedDefaultAdminRules.selector);
        AccessControlDefaultAdminRulesMock(_mock())._internal_grantRole(DEFAULT_ADMIN_ROLE, alice);
    }
    ...
    function testRevert_beginDefaultAdminTransferByNonDefaultAdmin() public {
        ...
        AccessControlDefaultAdminRulesMock(_mock()).beginDefaultAdminTransfer(bob);
    }
    ...
    function test_setPendingDefaultAdminAndSchedule() public {
        ...
        AccessControlDefaultAdminRulesMock(_mock()).beginDefaultAdminTransfer(alice);
        (address pendingAdmin, uint48 schedule) = AccessControlDefaultAdminRulesMock(_mock()).pendingDefaultAdmin();
    ...
    function test_whenThereIsAPendingDelaychangeDefaultAdminDelayShouldApplyItToNextDefaultAdminTransfer() public {
        ...
        for (uint8 i; i < 3; ++i) {
            AccessControlDefaultAdminRulesMock(_mock()).changeDefaultAdminDelay(newDelay);
            (, uint48 effectSchedule) = AccessControlDefaultAdminRulesMock(_mock()).pendingDefaultAdminDelay();
            ...
            (address pendingAdmin, uint48 transferSchedule) =
                AccessControlDefaultAdminRulesMock(_mock()).pendingDefaultAdmin();
            assertEq(pendingAdmin, alice);
    ...
    function test_acceptAdminTransferByPendingAdmin() public {
        ...
        // Storage changes
        assertFalse(AccessControlDefaultAdminRulesMock(_mock()).hasRole(DEFAULT_ADMIN_ROLE, owner));
        assertTrue(AccessControlDefaultAdminRulesMock(_mock()).hasRole(DEFAULT_ADMIN_ROLE, alice));
        assertEq(AccessControlDefaultAdminRulesMock(_mock()).owner(), alice);
    ...
    function testRevert_cancelDefaultAdminTransferByNonDefaultAdmin() public {
        ...
        AccessControlDefaultAdminRulesMock(_mock()).cancelDefaultAdminTransfer();
    }
    ...
    function testRevert_renounceDefaultAdminByNonDefaultAdmin() public {
        AccessControlDefaultAdminRulesMock(_mock()).beginDefaultAdminTransfer(address(0));

        vm.warp(DELAY + 1);
        vm.expectRevert(IAccessControl.AccessControlBadConfirmation.selector);
        AccessControlDefaultAdminRulesMock(_mock()).renounceRole(DEFAULT_ADMIN_ROLE, alice);
    }
    ...
    function test_renounceDefaultAdminRole() public {
        ...
        AccessControlDefaultAdminRulesMock(_mock()).renounceRole(DEFAULT_ADMIN_ROLE, owner);

        assertFalse(AccessControlDefaultAdminRulesMock(_mock()).hasRole(DEFAULT_ADMIN_ROLE, owner));
        assertEq(AccessControlDefaultAdminRulesMock(_mock()).defaultAdmin(), address(0));
        assertEq(AccessControlDefaultAdminRulesMock(_mock()).owner(), address(0));
    }
    ...
    function test_internal_grantRoleToRecoverDefaultAdminAccess() public {
        ...
        AccessControlDefaultAdminRulesMock(_mock())._internal_grantRole(DEFAULT_ADMIN_ROLE, bob);
    }
    ...
    function testRevert_changeDefaultAdminDelayByNonDefaultAdmin() public {
        ...
        AccessControlDefaultAdminRulesMock(_mock()).changeDefaultAdminDelay(5 hours);
    }
    ...
    function test_changeDefaultAdminDelayBeginsChangeToNewDelay() public {
        ...
        for (uint8 i; i < 3; ++i) {
            ...
            uint48 capWait = AccessControlDefaultAdminRulesMock(_mock()).defaultAdminDelayIncreaseWait();
            ...
            AccessControlDefaultAdminRulesMock(_mock()).changeDefaultAdminDelay(newDefaultAdminDelay);

            // Assert
            (uint48 newDelay, uint48 schedule) = AccessControlDefaultAdminRulesMock(_mock()).pendingDefaultAdminDelay();
    ...
    function test_changeDefaultAdminDelayScheduleAgain() public {
        ...
        for (uint8 i; i < 3; ++i) {
            ...
            for (uint8 j; j < 3; ++j) {
                ...
                AccessControlDefaultAdminRulesMock(_mock()).changeDefaultAdminDelay(anotherNewDefaultAdminDelay);

                // Assert
                (uint48 newDelay, uint48 schedule) =
                    AccessControlDefaultAdminRulesMock(_mock()).pendingDefaultAdminDelay();
    ...
    function testRevert_rollbackDefaultAdminDelayByNonDefaultAdmin() public {
        ...
        AccessControlDefaultAdminRulesMock(_mock()).rollbackDefaultAdminDelay();
    }
    ...
    function test_rollbackDefaultAdminDelayResetsPendingDelayAndSchedule() public {
        ...
        for (uint8 i; i < 3; ++i) {
            ...
            AccessControlDefaultAdminRulesMock(_mock()).changeDefaultAdminDelay(12 hours);
