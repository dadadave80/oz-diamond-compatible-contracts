...
import "@diamond/proxy/diamond/libraries/DiamondErrors.sol";
import "@diamond/proxy/diamond/libraries/DiamondLogs.sol";
import {
    DIAMOND_STORAGE_SLOT,
    DiamondStorage,
    FacetCut,
    FacetCutAction
} from "@diamond/proxy/diamond/libraries/DiamondStorage.sol";
...
library LibDiamond {
    ...
    function _addFunctions(address _facetAddress, bytes4[] memory _selectors) internal {
        ...
        if (ds.facetToSelectors[_facetAddress]._length() == 0) _addFacet(ds, _facetAddress);
    ...
    function _addFunction(DiamondStorage storage _ds, bytes4 _selector, address _facetAddress) internal {
        if (!_ds.facetToSelectors[_facetAddress]._add(_selector)) {
    ...
    function _removeFunction(DiamondStorage storage _ds, address _facetAddress, bytes4 _selector) internal {
        ...
        if (!_ds.facetToSelectors[_facetAddress]._remove(_selector)) {
        ...
        if (_ds.facetToSelectors[_facetAddress]._length() == 0) {
